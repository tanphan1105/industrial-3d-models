# Industrial 3D Models — Landing Page

**Engineering-accurate 3D models** for water treatment systems — RO systems, filtration skids, and wastewater plants.

## Live Site
[→ View Landing Page](https://tanphan1105.github.io/industrial-3d-models/)

## Products Available On
- **Fab.com** — [fab.com/sellers/Trong Tan Phan](https://www.fab.com/sellers/Trong%20Tan%20Phan)
- **CGTrader** — [cgtrader.com/designers/tanphan1105](https://www.cgtrader.com/designers/tanphan1105)

## Tech Stack
- Pure HTML/CSS/Vanilla JS (Zero-dependency)
- Apple-grade B2B UI with Bento Grid layout
- JSON-LD SEO Schema for Google Rich Results
- Sub-1s load time

## ☕ Support the Designer
Creating engineering-grade, real-world scale 3D models takes significant time and precision. If these open assets helped you close a deal or finish a rendering project, consider buying me a coffee!

[![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/phantrongtan)

## Contact
- Email: trongtan.p@icloud.com
- Zalo: 0985267326
