...
def render_big_square_logo(icon_path, target_size=126):
    """Render biểu tượng vuông chính hãng to đùng, siêu nét, bóng đổ 3D mềm mại"""
...
def create_pure_square_logos(base_image_path, output_path):
    ...
    # 3. 10 BIỂU TƯỢNG VUÔNG CHÍNH HÃNG TO BẰNG NHAU (KHÔNG NỀN, KHÔNG HỘP)
    icons_dir = r"D:\WT3D_Project\Z_Tools\Assets\Icons"
    square_logos = [
        os.path.join(icons_dir, "solidworks_cube_official.png"),
        os.path.join(icons_dir, "inventor_square.png"),
        os.path.join(icons_dir, "autocad_square.png"),
        os.path.join(icons_dir, "revit_square.png"),
        os.path.join(icons_dir, "fusion_square.png"),
        os.path.join(icons_dir, "catia_cube_official.png"),
        os.path.join(icons_dir, "blender.png"),
        os.path.join(icons_dir, "3dsmax_square.png"),
    ...
    for idx, icon_file in enumerate(square_logos):
        ...
        # Render logo to đều nhau
        ico_img = render_big_square_logo(icon_file, target_size=icon_size)
        overlay.paste(ico_img, (ix, iy), ico_img)
        
    final_img = Image.alpha_composite(base_img, overlay)
    final_img.save(output_path, "PNG")
    print(f"Pure Square Logos Generated: {output_path}")

if __name__ == "__main__":
    src = r"D:\WT3D_Project\Z_Tools\Demo_Watermark_3D_Bevel_Edge_2K.png"
    out = r"D:\WT3D_Project\Z_Tools\Demo_Pure_Square_Logos_2K.png"
