...
#
# Changelog v3.2 (so với v3.1):
# [FIX-1]  _apply_material_profile() — sửa NameError: node_name undefined in scope
# [FIX-2]  smart_render() — thêm _preserve_camera() để tránh camera drift giữa frames
# [FIX-3]  _make_render_opts() — passes lấy từ mode_cfg thay vì hardcode
# [FIX-4]  _run_batch() — setup_environment() chỉ gọi 1 lần (ngoài loop) → tăng tốc
# [FIX-5]  get_scene_bounds() — bọc transform multiply trong try/except riêng, fallback an toàn
# [FIX-6]  apply_materials() PASS 2 — per-node fallback khi batch fail
# [FIX-7]  _guess_profile_from_inventor_material() — bỏ extrapolate b=g không chính xác
# [FIX-8]  setup_camera() Ortho — dùng lux_safe() thay vì bare call
# [FIX-9]  _run_setup() — guard khi snapshot load fail, vẫn chạy apply_materials()
# [FIX-10] RenderContext — thêm __post_init__ validation để bắt lỗi sớm
# [OPT-I]  _run_batch() — precompute cam list, log ETA per frame
# [OPT-J]  setup_environment() — thêm guard không gọi lại nếu env đã set (flag)
# [FINAL-FIX] Áp dụng trực tiếp Hard Rough Plastic Black cho phụ kiện để triệt tiêu lỗi màu
# ==============================================================================
        ...
        try:
            ...
            try:
                ...
                if active_env:
                    ...
                    log("LIGHT", f"  [Adaptive Optics] B={brightness}, Rot=115°, max_dim={max_dim:.1f}")

                    try:
                        active_env.setBackgroundColor([255, 255, 255])
                        active_env.setGroundShadows(True)
                        active_env.setOcclusionGroundShadows(True)
                        active_env.setGroundReflections(True)
                        log("LIGHT", "  [Studio] White BG + Shadows + Reflections ON")
                    except Exception as e:
...
def smart_render(cam_name: str, out_path: str, ctx: RenderContext,
    ...
    # [FIX-2] Bọc toàn bộ render trong _preserve_camera để tránh camera drift
    with _preserve_camera():
        ...
        if DRAFT_PRECHECK and ctx.quality in ("FINAL", "ULTRA"):
            ...
            lux.renderImage(draft_path, w // 6, h // 6,
...
def main() -> None:
    _validate_config()
    log("HEAD", "NIGHT-CRAWLER v3.2 — COMMERCIAL RENDER OS BOOTING")
    ...
    print("─" * 60)
    log("HEAD", "NIGHT-CRAWLER v3.2 — SESSION END")
