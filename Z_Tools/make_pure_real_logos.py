...
def render_real_transparent_logo(icon_path, target_size=120):
    """Render logo hãng trong suốt 100%, to rõ, không nền, có bóng đổ 3D nhẹ"""
...
def create_pure_real_logos(base_image_path, output_path):
    ...
    # 3. 10 LOGO CHÍNH HÃNG GỐC SIÊU TO (TRONG SUỐT 100%, KHÔNG NỀN HỘP)
    icons_dir = r"D:\WT3D_Project\Z_Tools\Assets\Icons"
    real_logos = [
        os.path.join(icons_dir, "solidworks.png"),
        os.path.join(icons_dir, "inventor_real.png"),
        os.path.join(icons_dir, "autocad.png"),
        os.path.join(icons_dir, "revit_real.png"),
        os.path.join(icons_dir, "fusion_real.png"),
        os.path.join(icons_dir, "catia_real.png"),
        os.path.join(icons_dir, "blender.png"),
        os.path.join(icons_dir, "3dsmax_real.png"),
    ...
    for idx, icon_file in enumerate(real_logos):
        ...
        # Render logo chính hãng siêu nét
        ico_img = render_real_transparent_logo(icon_file, target_size=icon_size)
        overlay.paste(ico_img, (ix, iy), ico_img)
        
    final_img = Image.alpha_composite(base_img, overlay)
    final_img.save(output_path, "PNG")
    print(f"Pure Real Logos Generated: {output_path}")

if __name__ == "__main__":
    src = r"D:\WT3D_Project\Z_Tools\Demo_Watermark_3D_Bevel_Edge_2K.png"
    out = r"D:\WT3D_Project\Z_Tools\Demo_Pure_Real_Logos_2K.png"
    create_pure_real_logos(src, out)
