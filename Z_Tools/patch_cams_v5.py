...
def get_cam_1():
    ...
    oCam.Eye = tg.CreatePoint(cx + R*0.5, cy, cz + R) 
    ...
    oCam.Target = tg.CreatePoint(cx, cy + dy*0.2, cz)"""),
        # Cut 4: Dutch Angle Slide (60f) -> Slide across front
        ("Dutch Angle Slide", 60, "Shaded", "0.65", """
    Dim t = i / fMax : Dim e = t * t * (3.0 - 2.0*t)
    Dim R = max_dim * 1.5
    oCam.Eye = tg.CreatePoint(cx - R*0.5 + R*1.0*e, cy, cz + R*0.8)
    oCam.Target = tg.CreatePoint(cx, cy + dy*0.3, cz)
    Dim tilt = Math.Sin(t * Math.PI) * 0.15
    oCam.UpVector = tg.CreateUnitVector(orgUpX + tilt, orgUpY, orgUpZ) """),
        # Cut 5: Sky Drop Hero (70f) -> Front
        ("Sky Drop Hero", 70, "Shaded", "0.7", """
    Dim t = i / fMax : Dim e = 1.0 - Math.Pow(1.0 - t, 4.0)
    Dim R = max_dim * (1.2 - 0.5 * e)
    Dim ang = Math.PI * 0.3 + e * Math.PI * 0.15
    oCam.Eye = tg.CreatePoint(cx + R*Math.Cos(ang), rb.MaxPoint.Y + max_dim*(1.2 - 1.4*e), cz + R*Math.Sin(ang))
    oCam.Target = tg.CreatePoint(cx, cy + dy*0.2, cz) """),
        # Cut 6: Worm Eye Flyby (60f) -> Slide front floor
        ("Worm Eye Flyby", 60, "Shaded", "0.8", """
    Dim t = i / fMax : Dim e = t
    Dim R = max_dim * 0.8
    oCam.Eye = tg.CreatePoint(cx - max_dim*0.8 + 1.6*max_dim*e, minY - dy*0.1, cz + R)
    oCam.Target = tg.CreatePoint(cx, cy + dy*0.3, cz) """),
...
def get_cam_3(): # SHORTS 9:16 (10 scenes)
    ...
    oCam.Eye = tg.CreatePoint(cx + R*0.5, cy, cz + R*0.8)
    oCam.Target = tg.CreatePoint(cx, cy + dy*0.15, cz) """),
        ("Extreme Punch", 40, "Shaded", "0.75", """
    Dim t = i / fMax : Dim e = Math.Pow(t, 4.0)
    Dim R = max_dim * (1.2 - 0.8 * e)
    oCam.Eye = tg.CreatePoint(cx, cy - dy*0.2 + dy*0.2*e, cz + R)
    oCam.Target = tg.CreatePoint(cx, cy + dy*0.2, cz) """),
...
def generate_code(scenes, w, h):
    ...
    for idx, (name, frames, mode, persp, logic) in enumerate(scenes, 1):
        ...
        if "oCam.UpVector =" not in logic:
            lines.append("    oCam.UpVector = tg.CreateUnitVector(orgUpX, orgUpY, orgUpZ)")
