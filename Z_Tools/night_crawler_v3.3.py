# -*- coding: utf-8 -*-
# ==============================================================================
# 🚀 NIGHT-CRAWLER KEYSHOT BATCH RENDER ENGINE v3.3 — MASTER EDITION
...
#          _preserve_camera() chỉ còn dùng trong setup_camera() + SnapshotManager
...
# [WARN-3] DRAFT_PRECHECK — chia 4 thay chia 6 (960×540 thay 640×360)
# [WARN-4] Xóa hàm chết _get_scene_center_via_bbox()
# [WARN-5] Xóa _detect_transparent_housing() không dùng
# [WARN-6] Lighting preset — log tên preset được chọn
# ==============================================================================
...
class RenderContext:
    ...
    def from_mode(cls, mode_cfg: Dict) -> "RenderContext":
        return cls(quality=mode_cfg["quality"], width=mode_cfg["width"], height=mode_cfg["height"])
...
# ==============================================================================
# ▶ CAMERA SYSTEM
# [BUG-3] _CAMERA_DEF_MAP — dict 1 lần ngoài function
# ==============================================================================
CAMERA_DEFS = [
...
# [BUG-3] Tạo dict 1 lần ở module level — không dict() lại trong vòng lặp
_CAMERA_DEF_MAP: Dict[str, Tuple[float, float]] = dict(CAMERA_DEFS)
...
def _preserve_camera():
    ...
    Chỉ dùng trong setup_camera() và SnapshotManager — KHÔNG bọc renderImage().
    """
    active = None
    try:
...
def setup_camera() -> None:
    log("CAM", "Setup Camera System v3.3…")
    ...
    for name, _ in CAMERA_DEFS:
        ...
        try:
            ...
            if "Ortho" in name:
                ...
                # [BUG-3] Dùng _CAMERA_DEF_MAP thay vì dict(CAMERA_DEFS)
...
# [BUG-1] setBackgroundColor float 0..1
# [WARN-1] HDRI rotation theo camera azimuth
# [WARN-6] log tên preset được chọn
# ==============================================================================
...
def setup_environment(active_camera: str = "B2B_01_Top_FrontRight", force: bool = False) -> None:
    """Setup HDRI + Lighting Preset chuẩn B2B.
    Chỉ chạy 1 lần trừ khi force=True.
    """
    ...
    if env is not None:
        try:
            ...
            try:
                ...
                if active_env:
                    # [WARN-1] Rim light 135° phía sau camera — tính theo azimuth
                    cam_az = _CAMERA_DEF_MAP.get(active_camera, (45.0, 0.0))[0]
                    ...
                    log("LIGHT", f"  [Adaptive Optics] B={brightness}, Rot={hdri_rotation:.1f}° (rim), max_dim={max_dim:.1f}")

                    try:
                        # [BUG-1] Float 0..1 — KS2025.1 không nhận int 0..255
                        ...
                        log("LIGHT", "  [Studio] White BG (1.0,1.0,1.0) + Shadows + Reflections ON")
                    except Exception as e:
    ...
    try:
        ...
        if studio_preset:
            ...
            log("LIGHT", f"  Lighting Preset selected: '{studio_preset}'")
        else:
            log("WARN", "  Không tìm được Lighting Preset phù hợp")
    except Exception as e:
        log("WARN", f"getLightingPresets: {e}")
...
_LIBRARY_SEARCH_MAP: Dict[str, List[str]] = {
    ...
    "Polished Stainless Steel":  ["Stainless Steel Polished", "Steel Polished", "Anodized Aluminum Polished"],
    "Cast Iron":                 ["Iron Rough", "Iron Brushed", "Steel Rough"],
    "Brass":                     ["Brass Polished", "Brass Brushed"],
    "Plastic Fitting Dark Gray": ["Paint Matte Grey", "Hard Textured Plastic Grey", "Hard Rough Plastic Black"],
...
# [WARN-2] Bổ sung entry còn thiếu trong v3.2
_SHADER_MAP: Dict[str, Any] = {
    ...
    "Polished Stainless Steel":  getattr(lux, "SHADER_TYPE_BRUSHED",      None),  # [WARN-2]
    "Cast Iron":                 getattr(lux, "SHADER_TYPE_METAL",        None),
    "Brass":                     getattr(lux, "SHADER_TYPE_METAL",        None),
    "Plastic Fitting Dark Gray": getattr(lux, "SHADER_TYPE_PLASTIC",      None),  # [WARN-2]
    "Plastic Rough Light Gray":  getattr(lux, "SHADER_TYPE_PLASTIC",      None),
    "Plastic Pipe Light Gray":   getattr(lux, "SHADER_TYPE_PLASTIC",      None),
...
def _guess_profile_from_inventor_material(node_name: str, mat_name: str) -> Optional[str]:
    ...
    def _get_color_variant(base_type: str) -> Optional[str]:
        ...
        if not has_color:
            return None
        if b < 0:
            return "Plastic Rough Light Gray" if base_type == "Plastic" else None
    ...
    is_metal = any(k in mat_name for k in ("steel", "iron", "cast", "metal", "thep",
                                            "gang", "nhom", "aluminum"))
    if is_metal:
...
def apply_materials() -> None:
    ...
    for node in _collect_scene_nodes():
        try:
            ...
            # Routing: Pipe → Pipe, Fitting/xám → Fitting Dark Gray
    ...
    if batch:
        try:
            ...
            log("WARN", f"Batch fail ({e}) — per-node fallback")
            per_ok = per_fail = 0
            for nid, mname in batch.items():
                try:
                    lux.applyMaterialMapping({nid: mname})
                    per_ok += 1
                except Exception as e2:
                    log("WARN", f"  Per-node fail [{nid}]: {e2}")
                    per_fail += 1
            log("MAT", f"  Per-node: ok={per_ok}, fail={per_fail}")
...
def _make_render_opts(ctx: RenderContext, cam_name: str,
                      mode_passes: Optional[List[str]] = None) -> Any:
    preset   = ctx.preset()
...
# ==============================================================================
# ▶ SMART RENDER
# [BUG-2] renderImage() KHÔNG nằm bên trong _preserve_camera()
#         Camera set trực tiếp, không restore trong lúc render
# [WARN-3] Draft precheck ÷4 thay ÷6 → 960×540 dễ detect lỗi vật liệu hơn
# ==============================================================================
def smart_render(cam_name: str, out_path: str, ctx: RenderContext,
    ...
    if DRAFT_PRECHECK and ctx.quality in ("FINAL", "ULTRA"):
        ...
        log("REND", f"  Draft ({w//4}×{h//4}) → {os.path.basename(draft_path)}")
        ...
        try:
            ...
            if "asset" in js:
                if "generator" in js["asset"]:
                    js["asset"]["generator"] = "Tanphan OS v3.3"
                    modified = True
            ...
            with open(filepath, 'wb') as f:
                f.write(out_data)
            log("INFO", "  Scrubbed GLB metadata (Generator: Tanphan OS v3.3)")
        except Exception as e:
...
class SnapshotManager:
    ...
    def _read_cameras(self) -> Dict:
        result: Dict = {}
        try:
            # _preserve_camera vẫn hợp lệ ở đây — chỉ đọc, không render
...
def _run_setup(ctx: RenderContext) -> None:
    if SNAPSHOT_MODE == "LOAD":
        ...
        if not loaded:
            log("WARN", "Snapshot load fail — fallback apply_materials()")
            apply_materials()
    else:
        apply_materials()
...
def _run_batch(ctx: RenderContext, mode_cfg: Dict) -> None:
    ...
    # setup_environment 1 lần trước loop
